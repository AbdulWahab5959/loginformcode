    ...
    if ($email != '') {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $password = rand();
                // update in database for that email

                $body = "Your new password:".$password;
                $headers = "From: abdulwahabk620@gmail.com";
                if (mail($to_email, $subject, $body, $headers)) {
                    $sql1 = "UPDATE `registeration` SET `password` = '$password' WHERE email='$email'";
            ...
            echo "<script>alert('Account on this Email does not exist')</script>";
        }
    } else {
            ...
            <div class="col-md-5 p-0 bg-black text-white" style="height: 100vh;">
                ...
                <div class="justify-content-center align-content-center d-flex text-center text-md-left">
                    <form method="POST" action="">
                        ...
                        <div class="d-flex">
                            <button class="btn mt-1 w-100  btn-primary" name="forget" type="submit">Submit</button>
                        </div>
                    </form>
                </div>
            </div>
